let handler = async (m, { conn, text, usedPrefix }) => {
  ...
  try {
    ...
    if (user === m.sender)
      return conn.reply(m.chat, `Tidak bisa mempunyai bestie dengan diri sendiri`, m);
    //if (user === conn.user.jid)
    //return conn.reply(m.chat, `Tidak bisa mempunyai bestie dengan bot`, m);
    ...
    if (
      ...
      conn.reply(
        m.chat,
        `Kamu sudah mempunyai bestie dengan @${
      ...
      conn.reply(
        m.chat,
        `Selamat anda resmi mempunyai bestie dengan @${
          user.split("@")[0]
...
handler.help = ["bf"].map((v) => v + " *@tag*");
handler.tags = ["bestfriend"];
handler.command = /^(bf)$/i;
handler.group = true;
handler.limit = true;
handler.fail = null;
